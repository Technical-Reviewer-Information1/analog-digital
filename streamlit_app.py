...
st.info("""
**まとめ**: 
- **アナログ**: 連続的な量で自然な表現が可能だが、ノイズの影響を受けやすい
- **デジタル**: 「0」「1」の不連続な量でノイズに強く、完全な復元が可能

どちらもそれぞれの特徴を活かした利用方法があります！
""")
...
st.info("""
**まとめ**: 
- **アナログコピー**: 物理的な情報の転写のため劣化が伴うが、直接的でシンプル
- **デジタルコピー**: 0と1の完全な複製で原理的に劣化しないが、変換処理が必要

それぞれの特性を理解して適切に使い分けることが大切です！
""")
...
st.info("""
**まとめ**: 
- **デジタル表現**: 「0」と「1」の組み合わせで正確な情報表現が可能
- **アナログ表現**: 連続的な量で無限の細かさを表現可能

ビット数が1つ増えるだけで、デジタルの表現能力は2倍に！
一方、アナログは制限なく連続的な値を表現できます。
""")
...
st.markdown("アナログとデジタルでは、データ圧縮の方法も違います。それぞれの特徴を理解してみましょう！")
...
with col1:
    ...
    if quality < 5:
        ...
        st.caption("アナログの特徴：圧縮するほど情報が失われる")
    else:
        st.success("高音質です")
        st.caption("アナログの強み：リアルで温かみのある音質")
...
with col2:
    ...
    if compression_type == "可逆圧縮":
        ...
        st.caption("デジタルの強み：数学的な処理で完全復元")
    elif compression_type == "非可逆圧縮":
        st.info("大幅サイズ削減（品質は少し犠牲）")
        st.caption("人間の知覚に影響しない範囲で情報を削減")
    else:
        st.info("最高品質")
        st.caption("アナログの強み：変換なしの直接的な表現")
...
categories = ['ノイズ耐性', 'コピー耐性', '圧縮効率', 'シンプルさ', '自然表現', 'リアルタイム性']
analog_scores = [2, 1, 2, 5, 5, 5]  # Analog has its strengths
digital_scores = [5, 5, 4, 3, 3, 4]  # Digital has different strengths
...
**デジタルの強み** 💻
- **ノイズ耐性**: 0と1の判別ができるため、ノイズに強い
- **コピー耐性**: 完全複製が可能で劣化しない
- **圧縮効率**: 可逆圧縮により効率的にデータを圧縮

**アナログの強み** 📻
- **シンプルさ**: 複雑な変換処理が不要で直接的
- **自然表現**: 連続的な変化を自然に表現
- **リアルタイム性**: 遅延なく即座に情報を伝達

🌟 どちらにも優れた特徴があり、用途や状況によって使い分けることが重要です！

**使い分けの例**
- 🎵 音楽：アナログの温かみ vs デジタルの高品質
- 📷 写真：フィルムの風合い vs デジタルの便利さ
- 📺 映像：アナログの自然さ vs デジタルの鮮明さ
""")
